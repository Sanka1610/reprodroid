...
**Phase 4.8実装・受入作業中（2026-09-12）:** Android／Runnerのbounded log export、Room14／SQLite8以降のmigration回帰、debug application ID分離、固定unsigned CI、隔離手動署名、SBOM、license／privacy監査とPhase 4 retained gapの最終照合を進めています。旧debugのuninstallでは登録・設定・履歴・pairingが失われ、releaseで再登録・再pairingが必要です。自動uninstall、push、`main`統合、公開は行いません。契約は[Phase 4.8 release hardening](../reprodroid-project/docs/design/phase-4-release-hardening-contract.md)です。
...
**Phase 4.7実装・製品受入完了（2026-09-09）:** public `codeberg.org`をGitHubと並ぶ閉じたproviderとして追加し、provider ID付き登録、SHA-1 object format確認、上限付きsource discovery、release／attachment metadata、厳密なdownload URL・MIME・size・SHA-256・signed standalone APK検査、明示的な複数APK選択、version付き保存条件、Room22のprovider-nullabilityとdownloaded-content履歴fork、Codeberg metadata-only定期確認を実装しました。Android debug／release JVMは各190件、lint／assemble、API 36でのRoom21→22 migrationとdownload CAS計3件がPASSです。製品UIでは`UnifiedPush/android-example`の登録から複数APK選択・検査・cold start復元、および`qwerty287/ftpclient` 3.2.0の公式APK検査、独立Build A／B、raw三軸comparisonの保存とcold-start復元を確認しました。raw outcomeはOfficial vs A `DIFFERENT`、Official vs B `DIFFERENT`、A vs B `MATCH`で、`reproducible=false`、`trustEligible=false`、`installEligible=false`です。Codeberg generic buildはRunnerの`generic-build@1`／`apk-comparison@1`／`codeberg-source@1`をJob作成前に必須とし、scheduled checkはAPK取得、Runner、toolchain、build、comparison、trust、installを開始しません。自動検証と製品経路の実施結果は[Phase 4.7実装記録](../reprodroid-project/reports/2026/09/2026-09-08-phase-4-7-implementation.md)を正本とします。
...
4.7は[provider契約](../reprodroid-project/docs/design/phase-4-codeberg-provider-contract.md)に従い、AndroidをRoom22、RunnerをSQLite12のまま`codeberg-source@1`対応へ進めました。GitHub／Codebergのwire DTO・API／rate limit・download policyはprovider別に閉じ、共通domain model、metadata observation schema 2、saved selection condition、downloaded-content forkだけを共有します。新規generic Jobは`docker-generic-v2`を表示し、`/tmp` 960 MiB `noexec`とnative展開専用`/run/reprodroid-native` 64 MiB `exec`を合計1 GiBとして証拠化します。既存`docker-generic-v1`証拠も同じ公開上限・隔離境界のhistorical recordとして読み取れますが、未知profileや同一comparison内のv1→v2変化はfail closedです。`UnifiedPush/android-example`では製品UI上の登録、source discovery、release 2.3.1、2候補からの`UP-Example-main.apk`明示選択、download／signed APK inspection、cold-start persistenceを確認しました。`qwerty287/ftpclient`ではrelease 3.2.0の公式APKを取得し、package、version、signer、計算済みSHA-256を検査しました。API 36 instrumentationではRoom21→22 migrationとstale download failure CASを計3件実行してPASSです。

自動検証はdebug／release JVM各190件（合計380件、failure／error／skip 0）、`lintDebug`（error 0、warning 33、hint 1）、debug／unsigned release／debug AndroidTest APK生成がPASSです。Codeberg Build Aの旧実行はSDK package directory不一致を`SANDBOX_TOOLCHAIN_MISMATCH`でfail closedに停止し、その失敗を公式asset履歴へ不可逆に書き込まず再実行可能にする復旧経路を確認しました。API 37 platformをcatalog、inventory、host preflight、Docker preflightで正規の`android-37.0`へ揃え、`qwerty287/ftpclient`で独立Build A／Bを完走しました。raw三軸はOfficial vs A `DIFFERENT`、Official vs B `DIFFERENT`、A vs B `MATCH`として保存され、cold start後も製品UIへ復元されます。fixture／JVM／compile／migration結果ではなく実製品経路を受入証拠としています。

計画範囲はpublic GitHub／Codebergのsource-onlyを含むGradle登録、汎用build／comparison、不足toolchain導入、history／手動cleanup／監査export、定期確認・通知、release HTTPS／pairing、Room／SQLite migration、Android／Runnerのlog export、署名releaseとlicense・privacy対応です。GitLabは将来候補。Play Store／F-Droid配布・適合性評価、Google Play services、共有用診断・自動送信は対象外です。

初期受入はAndroid 16・debug＋明示的development loopback HTTP＋ADB reverseを互換経路として維持し、paired secure modeとreleaseはADB経由でもHTTPSを要求します。定期確認は1時間刻み・既定6時間または毎日指定時刻、releaseのみ／非従量制のみ／充電条件なしを既定とし、全体／app OFFと個別設定を持ちます。定期確認はmetadata・通知までで、既存15分周期のJob同期とは別です。承認済み比較の資源不足に限る自動増量・再試行は別契約・既定OFFとし、Job別scan reviewと全体budgetを維持します。4.6は利用者承認により実装・検証・ローカル`develop`統合までを対象とし、push／`main`統合／公開は行いません。
