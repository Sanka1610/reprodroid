...
4.7は[provider契約](../reprodroid-project/docs/design/phase-4-codeberg-provider-contract.md)に従い、AndroidをRoom22、RunnerをSQLite12のまま`codeberg-source@1`対応へ進めました。GitHub／Codebergのwire DTO・API／rate limit・download policyはprovider別に閉じ、共通domain model、metadata observation schema 2、saved selection condition、downloaded-content forkだけを共有します。`UnifiedPush/android-example`では製品UI上の登録、source discovery、release 2.3.1、2候補からの`UP-Example-main.apk`明示選択、download／signed APK inspection、cold-start persistenceを確認しました。`qwerty287/ftpclient`ではrelease 3.2.0の公式APKを取得し、package、version、signer、計算済みSHA-256を検査しました。API 36 instrumentationではRoom21→22 migrationとstale download failure CASを計3件実行してPASSです。

自動検証はdebug／release JVM各189件（合計378件、failure／error／skip 0）、`lintDebug`（error 0、warning 33、hint 1）、debug／unsigned release／debug AndroidTest APK生成がPASSです。Codeberg Build Aの旧実行はSDK package directory不一致を`SANDBOX_TOOLCHAIN_MISMATCH`でfail closedに停止し、その失敗を公式asset履歴へ不可逆に書き込まず再実行可能にする復旧経路を確認しました。API 37 platformをcatalog、inventory、host preflight、Docker preflightで正規の`android-37.0`へ揃えるRunner修正後の独立Build A／Bとraw三軸comparisonは、実際に完走するまで未受入として分離します。fixture／JVM／compile／migration結果を製品build成功へ読み替えません。
