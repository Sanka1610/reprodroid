...
class LiveSandboxComparisonTest {
    @Test fun runsExplicitComparisonStepThroughProductionRepositories() = runBlocking(Dispatchers.IO) {
        ...
        val appId = if (action == "start") {
            ...
            assertEquals("6.1.4", preview.release.tagName)
            assertEquals(expectedCommit, preview.resolvedCommitSha)
            repository.registerAndDownload(preview, ManagementMode.VERIFICATION, InstallationSource.OFFICIAL_RELEASE, false)
